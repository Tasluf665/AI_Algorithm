...
def main():
    ...
    explored_order, solution_path = depth_first_search(maze_obj)
    end_time = time.time()  # Record the end time
    execution_time = end_time - start_time  # Calculate execution time

    explorer_agent = agent(maze_obj, footprints=True, shape='square', color=COLOR.green)
    maze_obj.tracePath({explorer_agent: explored_order}, showMarked=True)
    solver_agent = agent(maze_obj, footprints=True, color=COLOR.yellow)
    maze_obj.tracePath({solver_agent: solution_path})

    textLabel(maze_obj, 'DFS Path Length', len(solution_path) + 1)
    textLabel(maze_obj, 'DFS Search Length', len(explored_order))

    print('DFS Path Length', len(solution_path) + 1)
    print('DFS Search Length', len(explored_order))
